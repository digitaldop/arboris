...
def payload_debug_report(payload, *, source, document_id, max_depth=5, max_list_items=2, include_attachment=False):
    ...
    if include_attachment:
        attachment_report, supplier_from_xml = attachment_debug_report(payload, timeout=_api_timeout())
        attachment_report["attachment_supplier_fields_present"] = _presence_map(supplier_from_xml, SUPPLIER_FIELDS)
        report["attachment_analysis"] = attachment_report
    else:
        report["attachments_metadata"] = attachment_metadata(payload)
    return report
